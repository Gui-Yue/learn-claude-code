...
| assemble_tool_pool | 組み込みツールと MCP ツールを一つのプールに統合 |
| mcp\_\_server\_\_tool 命名 | 異なる server 間のツール名衝突を防止 |

s18 の教学版 worktree 隔離、自動認領、空き時ポーリング、プロトコルシステムを踏襲。本章の追加：`connect_mcp` ツール — 外部サービスに接続、ツールを発見、ツールプールに統合。
...
プレフィックス `mcp__{server}__{tool}` で異なる server 間のツール名衝突を防止。名前は `normalize_mcp_name` でサニタイズ。
...
| ツールプール | 固定 BUILTIN_TOOLS | assemble_tool_pool が動的に mcp\_\_ プレフィックスツールを統合 |
| 名前の安全性 | なし | normalize_mcp_name サニタイズ |
